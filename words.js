const WORDS = [
    ...
    function processText(text) {
        ...
        for (var i = 0; i < text.length; i++) {
            currentLetterIndex = wordOfTheDay.indexOf(text[i]);
...
1. Creates a new array called colors.
2. Loops through the word of the day.
3. If the letter is not in the word of the day, set the color to gray.
4. If the letter is in the word of the day, but is not in the correct spot, set the color to yellow.
5. If the letter is in the word of the day, and is in the correct spot, set the color to green.
6. Set the colors for each letter. */
    ...
    function colorKeyboard(colorsByKeyCode) {
        for (var i = 0; i < colorsByKeyCode.length; i++) {
            ...
            key = document.getElementById("a-keyboard-" + colorsByKeyCode[i].letter).parentElement.querySelectorAll("a-entity")
            //set background color to colorsByKeyCode[i].color
            key[0].setAttribute("material", {
                "color": colorsByKeyCode[i].color
            });
            document.getElementById("a-keyboard-" + colorsByKeyCode[i].letter).setAttribute("color", "#000")
        }
    }
    /* Here is the explanation for the code above:
1. First we loop through each item in the colorsByKeyCode array.
...
2. Inside the switch statement, we check to see if the user has pressed the backspace key. If they have, we remove the last letter from the input.
3. We check to see if the user has pressed the enter key. If they have, we check to see if the input is a valid word.
4. We check to see if the user has pressed the space key. If they have, we add the space to the input.
5. We check to see if the user has pressed any other key. If they have, we add the key's value to the input.*/
